...
- Under the unsupported `spec.mode: next` dev toggle, additionally the A2A playground stack (NATS, bus provisioning, the auth callout, the A2A gateway, the capability verifier) — see the [PlatformAgent CRD page](/kube-agents/operator/platformagent-crd/) for what it renders.
