...
- It also reserves the agent's A2A bus credential against user-authored containers, by name and by source. A volume in `extraVolumes`/`sidecarVolumes` named `a2a-bus-token`, or a mount in `initContainers`/`sidecars`/`extraVolumeMounts` naming it, is `Forbidden`; so is any volume — whatever it is called — that projects a ServiceAccount token for the `a2a-bus` audience or references the `<agent>-a2a-nats-creds` Secret, as either a `secret` volume or a projected `secret` source. The agent's ServiceAccount token is pod-scoped and the bus's auth callout resolves the Pod's ServiceAccount, so a second container holding either one authenticates as the agent; this reservation is a guard against that misconfiguration rather than a boundary against a container that already runs in the Pod. Reading the same Secret through `env.valueFrom.secretKeyRef` is untouched — that is the supported route for the Hermes bridge sidecar. The controller strips a reserved volume and its mounts from the render as well, which is what holds on an install where the webhooks are off or `failurePolicy: Ignore` swallows them.
