...
- A `hostPath` entry on either volume list is refused at admission, and on an install where the
  webhook did not run (the Helm chart ships it off; an enabled one fails open at
  `failurePolicy: Ignore`) the controller leaves it out of the Pod at render, together with every
  `volumeMount` naming it on the agent container, the dashboard container, and the CR's own
  `sidecars`/`initContainers`. The rest of the Pod renders as written, and the `VolumesDropped`
  condition ([`status`](#status) below) names what was left out.
...
Five condition types appear in `conditions`, and only the first is always present:
...
| `VolumesDropped`      | Only while `spec.deployment.extraVolumes` or `.sidecarVolumes` carries a `hostPath` | `status: True`, `Reason: HostPathVolumeDropped`. The render left the named `hostPath` entries and every mount naming them out of the agent Pod; the message lists each by field and index. Not `Degraded`: the Pod runs without them. Remove the entries to clear it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
`Ready` condition, with any `Degraded`, `EventWatcher` or `VolumesDropped` condition written in the same pass, carries
