...
- A `hostPath` entry on either volume list is refused at admission, and on an install where the
  ...
  its default `failurePolicy: Ignore`) the controller leaves it out of the Pod at render, together
  with every `volumeMount` naming it on the agent container, the dashboard container, and the CR's
  own `sidecars`/`initContainers`. The rest of the Pod renders as written, and the `VolumesDropped`
  condition ([`status`](#status) below) names what was left out — as many entries as a condition
  message holds, and a count of any beyond that.
...
| `VolumesDropped`      | Only while `spec.deployment.extraVolumes` or `.sidecarVolumes` carries a `hostPath` | `status: True`, `Reason: HostPathVolumeDropped`. The render leaves the named `hostPath` entries and every mount naming them out of the agent Pod; the message lists them by field and index, and on a spec carrying more of them than a condition message may hold it lists as many as fit and counts the rest. Not `Degraded`: the Pod runs without them. Written on whichever status the pass ends on, `Ready` or `Degraded`, because the drop happens at render and three of the reasons that park `phase: Degraded` render first — `ShellSandboxKeysMissing` among them, which is where a chart-default install sits, and a chart-default install is one with no webhook in front of it. Remove the entries to clear it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
same shape: it exists only while there is something to report, and it is the render-side half of
the `hostPath` refusal, which the admission webhook makes at admission on installs where it runs.
...
`observedGeneration` is the generation at which that set last changed. A spec edit bumps `metadata.generation` at admission
