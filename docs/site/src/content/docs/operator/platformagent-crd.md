...
- A `hostPath` entry on either volume list is refused at admission, and on an install where the
  ...
  condition ([`status`](#status) below) names what was left out — as many entries as fit in 4096
  characters, then a count of the rest, and an entry longer than that on its own cut with `...`.
...
same shape: it exists only while there is something to report about the Pod that is running, and
