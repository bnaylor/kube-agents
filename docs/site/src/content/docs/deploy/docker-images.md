...
`k8s-operator/go.mod`; the inventory pins baked into the operator binary (the fluent-bit fallback,
and the NATS and nats-box defaults the `next` stack renders; the first-party `next` defaults are
not inventory entries, and no check reads them); the example manifests; and the kustomize
