...
| Whether the agent diagnoses a defect you planted for it                                    | `bench/tasks/<name>/task.yaml`                                                                   | `hack/ci-eval-pr.sh`, as the Prow presubmit; a `NIGHTLY_TASKS` entry runs only under `EVAL_TIER=nightly`, the nightly periodic's export                                                                                                                                                                                                                                                                                                                       | runs as a presubmit and reports on the pull request — except a `NIGHTLY_TASKS`-only case, which runs nightly and reports on no pull request; whether the presubmit blocks is Prow config this repository cannot read |
...
| The release gate                                                                           | `tests/e2e/`                                                                                     | `rc-release-pipeline.yml`, dispatched by `rc-scheduler.yml` on a three-hourly schedule; `nightly-pipeline.yml`, dispatched daily by `nightly-scheduler.yml`; `e2e-gchat-test.yml` by hand                                                                                                                                                                                                                                                                     | nothing — it gates releases, not pull requests                                                                                                                                                                       |
...
**The three paths-filtered workflows report `success` on a pull request that ran nothing.**
`k8s-operator-test.yml`, `agentplugins-test.yml`, and `a2a-test.yml` all run `dorny/paths-filter` and then gate
...
`PYTHON_TEST_DIRS`, a list of fifteen globs in the `Makefile`. A directory the globs miss fails
