...
| `a2a/README.md` | Component README | Front door for the integrated stage-1 A2A tree on `a2a/something-working`: what each component is and where it runs, which seams are stable (0.4 envelope, gateway Adapter interface) versus still moving (owed lists), and where the install runbook lives. | Component map, stable-vs-moving seams | Anyone building on the integration branch |
| `a2a/docs/hermes-bridge.md` | Design doc | The Hermes bridge: how the platform agent's running Hermes process joins the A2A bus as an executor — task fold, cancel and steer handling, shutdown, and the sidecar deployment story (`spec.deployment.sidecars`, set only with `mode: next`). | Bridge lifecycle, sidecar deployment, mode caveat | Contributors changing the bridge |
| `a2a/persona/README.md` | Component README | Why the A2A persona content (the `a2a-topics` skill) lives under `a2a/` rather than in the shipped persona tree: everything here is dark by construction, and the operator overlays the skill only under `mode: next`. | Skill overlay, dark-by-construction rule | Contributors adding A2A persona content |
| `a2a/persona/platform/skills/a2a-topics/SKILL.md` | Agent skill | The platform agent's blackboard skill: reading and writing the durable named topics on the A2A bus (upgrade readiness, environment blueprint, dated annotations). | Topic reads/writes via the `a2a` CLI | The platform agent, under `mode: next` only |
| `a2a/web/README.md` | Component README | Bring-up for the read-only web rail: port-forward to the NATS ws listener, connect as the `web` user, watch the live event sequence; includes the publish-refusal probe against the writerless subject. | Dev-only rail, port-forward transport, refusal probe | Developers running the rail against an install |
