...
| `docs/designs/spec-chatops-gateway.md` | Feature design | The chat-to-bus gateway: deterministic code with no model of its own, owning sessions (one backend conversation, one `contextId`), requester identity onto the bus as an advisory `authority` block, the group-chat substrate, Discord as the test backend, and the Slack adapter with its `user_id` principal map. | Session model, `authority` block, roster snapshots, `openDirect`, session lifecycle | Merged design of record; the gateway program is implemented (`a2a/gateway`, Discord and Slack adapters), session spawning dark behind `A2A_SPAWN_SESSIONS`; the operator renders the gateway under `mode: next` but not yet the Slack adapter's env |
