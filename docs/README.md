...
| The A2A gateway process's env (backend selection, Slack token pair, gchat relay and allowlist, display mode, addressee) | `a2a/gateway/config.go` (`FromEnv`) |
...
| LiteLLM egress policy: `litellm-policy`, the `enable-litellm-network-policy` and `otlp-collector-namespace` annotations | `k8s-operator/internal/controller/platformagent_litellm_policy.go` |
...
| `docs/designs/spec-chatops-gateway.md` | Feature design | The chat-to-bus gateway: deterministic code with no model of its own, owning sessions (one backend conversation, one `contextId`), requester identity onto the bus as an advisory `authority` block, the group-chat substrate, Discord as the test backend, the Google Chat adapter as the production ingress, and the Slack adapter with its `user_id` principal map. | Session model, `authority` block, roster snapshots, `openDirect`, session lifecycle, gchat adapter, slack adapter | Merged design of record; the gateway program is implemented (`a2a/gateway`) with Discord, Google Chat and Slack adapters, and the operator renders the gateway, its env and the session-spawn arming (`A2A_SPAWN_SESSIONS=true`) under `mode: next`, but not yet the gchat or Slack adapters' env |
