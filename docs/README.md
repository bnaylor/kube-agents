...
| The env the operator writes into a CR-authored A2A sidecar (`POD_NAMESPACE`, `A2A_CAPABILITY_REQUIRED`), and that these two names win over whatever the CR set | `a2aExecutorSidecarEnv` in `k8s-operator/internal/controller/platformagent_a2a_callout.go` (writer; the precedence is `mergeEnvVars` in `manifest_helpers.go`, whose second argument wins), read by `capabilityScope()` in `a2a/cmd/hermes-bridge/main.go` and by the bridge config's capability switch in `a2a/hermes-bridge/` |
