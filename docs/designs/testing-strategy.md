...
> **STATUS: draft.** Real today: unit tests, a gating integration tier, and a standing seeded fleet. Presubmit runs eighteen cases at three repetitions and has blocked merges since 2026-09-02; the release gate is one test. Nightly is a merged pipeline with the full E2E matrix and the staging promotion, dispatched daily by `nightly-scheduler.yml` when unpromoted candidates exist. The eval tier §4.4 describes now has its content in-tree — `NIGHTLY_TASKS` and the `EVAL_TIER=nightly` switch in `hack/ci-eval-pr.sh` — but no scheduled job exports the switch yet; the Prow periodic that would is in flight in `oss-test-infra`. Everything else here is the plan.
...
>
> **What is not.** The eval tier is no longer unwritten, but it is also not this pipeline: its cases and selection switch live in `hack/ci-eval-pr.sh` (`NIGHTLY_TASKS`, appended under `EVAL_TIER=nightly`; the array is the roster and is not re-counted here), to be run by a Prow periodic that is a separate job from this workflow's E2E matrix, in flight in `oss-test-infra` and not scheduled yet. The zero-cost landing tier for a new case is the unadmitted state in §4.2 and the volume argument is answered by the standing fleet making cases cheap, so neither came back. Upgrade-from-the-last-validated-release and hardware-specific cases are still unwritten.
...
Nightly's Drift cell was the open one, and the answer is that nothing looks at it. It does run merged code on a schedule, which is the precondition this section names for feeding Drift, but the precondition is not the whole requirement: Drift needs the same thing measured the same way, and what nightly runs is the pass/fail E2E matrix rather than judged eval cases. There are no per-domain scores to record, so there is no row to write. This is a decision rather than an omission, and the thing that would reopen it is the nightly eval actually running: its cases and tier switch are in-tree now (§4.4), the Prow periodic that would run them — a different job from `nightly-pipeline.yml`, whose matrix this table describes — is in flight in `oss-test-infra`, and once it runs on a schedule that tier has scores and should say **records** in a row of its own.
