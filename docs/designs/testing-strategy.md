...
Controller and webhook tests run against fake clients, with the exceptions in the operator's controller package that start a real API server through envtest (gated on `KUBEBUILDER_ASSETS`, which `make -C k8s-operator test` sets) for behaviour only a real API server exhibits: generation bookkeeping on the PlatformAgent status, the write-and-requeue behaviour of a CR parked on a refusal, the callout's RBAC, and the render-side `hostPath` drop landing in a Deployment the API server accepted with no webhook in front of it. Nothing else below the cloud e2e tier runs a real API server; that remaining gap is §4.1b's second-ranked seam.
