...
- **Author:** [@bnaylor]
- **Date:** 2026-08-24
- **Status:** merged design of record; the switch itself (enum, helper pair, skew path, managed-`.env` pin) is implemented in the operator, and the NATS/gateway render it gates is implemented at playground posture, session-pod spawning included - the agent-side bus surface is not yet
