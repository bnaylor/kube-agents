...
*injected* as steering - a follow-up `message` on the same `taskId`, forwarded to the
...
*publisher*, bound to the authenticated NATS connection.  Whether that's a server-stamped
...
- `requester.principal` is the identity in *our* trust domain - the string RBAC will
