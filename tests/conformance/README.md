...
Thirteen assertions currently fail. They are decorated `@known_violation`, which
...
| A3  | `…events` has no rendered writer (the executor's is derived)     | 1 **KV** | `test_A3_the_events_subject_has_no_rendered_writer`                       | static `worker` still holds `a2a.tasks.*.*.events` for every addressee (gke-labs/kube-agents#1316); closes when A5 gives the Hermes bridge its own principal                                                       |
...
| D5  | availability and cost are security properties                    | **3**    | `D5AvailabilityAndCostAreSecurityProperties` (docstring)                  | no per-tenant or per-principal budget, so no exhaustion event to assert the refusal of. Amended 9/16: two bus budgets exist now; the docstring records why neither satisfies this — one degrades, one is capacity  |
