...
139 mutations: 114 KILLED, 22 NOISY, three `must_survive` controls (one on the
...
make cheap. Note that the summary line the harness prints accounts for 136 of
the 139: a `must_survive` control's verdict is `SURVIVED (expected)`, which is
neither killed, noisy, nor a survivor. The run also prints `BASELINE POLLUTED`
if the suite is not green once every mutation has been restored, which did not
happen here and would invalidate every verdict after whatever caused it. Each
