...
195 mutations: 162 KILLED, 22 NOISY, eleven `must_survive` controls, zero
...
this paragraph exists to make cheap. The eleven controls are one on the
...
and would false-red a safe workflow if it were dropped, and eight over B4's
...
against `--mirror` must not catch, and a `git log --merges --max-count=1`,
whose flags begin `--m` and which the ladder of `--mirror` abbreviations must
not catch either. Each of those edits is the _safe_ spelling of something a
workflow here legitimately does, and the control is that the suite does not
object to it. Note that the summary line the harness prints accounts for 184
of the 195: a `must_survive` control's verdict is
