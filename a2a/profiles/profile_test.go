...
// TestShippedProfilesParse is the workstream's definition-of-done check: every
// profile config file in this package parses strictly against the field table
...
func TestShippedProfilesParse(t *testing.T) {
	...
	if len(entries) < 2 {
		t.Fatalf("expected at least the chat and specialist profiles, found %d files", len(entries))
	}
	...
	if len(seen) < 2 {
		t.Error("no specialist profile shipped alongside chat")
	}
}
...
// at stage 3, which means going through the API server's JSON.
func TestProfilesAreJSONRoundTrippable(t *testing.T) {
	data, err := configs.ReadFile("cluster-a2a-next-dev.yaml")
	if err != nil {
		t.Fatal(err)
	}
	p, err := Load("cluster-a2a-next-dev.yaml", data)
	if err != nil {
		t.Fatal(err)
	}
	...
	if !reflect.DeepEqual(p.Spec, back) {
