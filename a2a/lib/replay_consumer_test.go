...
// replayGrant is the TASKS half of the gateway's JetStream grant as
// gke-labs/kube-agents#1672 renders it (a2aGatewayJetStreamGrants), plus the
// ack, flow-control and inbox subjects beside it in the identity. withDelete
// adds the one subject that PR withholds and this change needs.
...
// flags that matter here: allow_direct (so the replay horizon is DIRECT.GET,
// the subject the grant names, not STREAM.MSG.GET), limits retention with
