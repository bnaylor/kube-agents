...
// set, which is what this is. Seed came off the list in gke-labs#1306: its
// $JS.API grant is now a2aSeedJetStreamGrants(), CREATE and INFO on the
// streams provisioning names. Worker came off it in #1393, the same way:
// a2aWorkerJetStreamGrants(), INFO/CONSUMER/DIRECT.GET on the four streams it
// touches. Gateway was the last, and the narrowing #1666 asked for took it:
// a2aGatewayJetStreamGrants(), the same verbs on TASKS and on the session
// registry it owns. The set is now EMPTY, which is the state this test exists
// to keep.
//
