...
// The residue, asserted so it cannot grow quietly. A1 leaves exactly three
// static principals and each has a reason recorded at its definition: worker
// (session pods carry no Kubernetes identity — A2 closes it), web (a browser
// can never hold one) and sys (a human). A fourth appearing here means someone
// added a principal without asking whether it could have an identity.
func TestTheStaticResidueIsExactlyTheThreeWithReasons(t *testing.T) {
	var got []string
	for _, id := range staticIdentities(identityTestAgent()) {
		got = append(got, id.user)
	}
	want := []string{"worker", "web", "sys"}
