...
const (
	...
	// The streams the worker's and the gateway's JetStream API grants name,
...
// inactive_threshold it sets -- measured for `worker` in
// a2aWorkerJetStreamGrants, identical here. And a push consumer's
...
// the worker's to the streams it uses and the gateway's to TASKS and its own
// session registry, all by name and by verb (a2aSeedJetStreamGrants,
// a2aWorkerJetStreamGrants, a2aGatewayJetStreamGrants), and provision has moved
